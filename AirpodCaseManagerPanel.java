//https://toplearn.com/ShowQuestion/72449/%D9%85%D8%B1%D8%AA%D8%A8-%D8%B3%D8%A7%D8%B2%DB%8C-%D9%85%D8%AD%D8%B5%D9%88%D9%84%D8%A7%D8%AA%D8%AA
//https://stackoverflow.com/questions/45490815/sorting-objects-in-an-arraylist-by-price-attributes
//https://www.freecodecamp.org/news/how-to-sort-a-list-in-java/
//https://www.javaguides.net/2023/10/java-sort-custom-objects-in-ascending-and-descending-order.html
//https://www.w3-farsi.com/posts/8289/search-for-strings-in-java/
//https://stackoverflow.com/questions/22213496/searching-a-list-of-items-with-a-specific-name
//https://docs.relewise.com/docs/examples/java/search/product-search-with-java.html
//https://www.geeksforgeeks.org/searching-algorithms-in-java/

import javax.swing.*;
import java.awt.*;
import java.util.List;
import java.util.stream.Collectors;
import java.util.Comparator;

// Class representing an Airpod case manager
class AirpodCaseManager {
    String name;
    double price;
    String imagePath;

    // Constructor to initialize the AirpodCaseManager
    AirpodCaseManager(String name, double price, String imagePath) {
        this.name = name;
        this.price = price;
        this.imagePath = imagePath;
    }
}

// Class representing the Airpod case manager panel
public class AirpodCaseManagerPanel extends JFrame {
    private JFrame previousFrame;
    private List<AirpodCaseManager> airpodCaseManagers;
    private JPanel panel;

    public AirpodCaseManagerPanel(List<AirpodCaseManager> airpodCaseManagers, JFrame previousFrame) {
        this.previousFrame = previousFrame;
        this.airpodCaseManagers = airpodCaseManagers;

        setTitle("AirpodCaseManagers");
        setSize(800, 600);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLocationRelativeTo(null); // Centering the window on the screen

        panel = new JPanel();
        panel.setLayout(new GridLayout(0, 3, 10, 10)); // Setting a grid layout with 3 columns
        panel.setBackground(Color.BLACK);

        JButton searchButton = new JButton("جستجو");
        JTextField searchField = new JTextField();
        searchField.setPreferredSize(new Dimension(200, 30));
        searchButton.addActionListener(e -> searchProducts(searchField.getText()));

        JButton sortButton = new JButton("مرتب‌سازی بر اساس قیمت");
        sortButton.addActionListener(e -> sortProductsByPrice());

        JPanel searchPanel = new JPanel();
        searchPanel.add(searchField);
        searchPanel.add(searchButton);
        searchPanel.add(sortButton);

        add(searchPanel, BorderLayout.NORTH);

        // Creating a main panel for the products and the back button
        JPanel mainPanel = new JPanel(new BorderLayout());
        JButton backButton = new JButton("برگشت");
        backButton.addActionListener(e -> {
            previousFrame.setVisible(true);
            dispose(); // Closing the current frame
        });

        mainPanel.add(backButton, BorderLayout.NORTH);
        mainPanel.add(new JScrollPane(panel), BorderLayout.CENTER);

        add(mainPanel, BorderLayout.CENTER);
        displayProducts(airpodCaseManagers);
    }

    // Constructor for the AirpodCaseManagerPanel with a different parameter
    public AirpodCaseManagerPanel(List<AirpodCase> airpodCases) {
    }

    // Method to display products
    private void displayProducts(List<AirpodCaseManager> CaseManagers) {
        panel.removeAll();
        for (AirpodCaseManager CaseManager : CaseManagers) {
            JPanel CaseManagerPanel = new JPanel();
            CaseManagerPanel.setLayout(new BorderLayout());

            JLabel nameLabel = new JLabel(CaseManager.name);
            JLabel priceLabel = new JLabel("تومان " + CaseManager.price);
            nameLabel.setFont(new Font("Arial", Font.BOLD, 15));
            priceLabel.setFont(new Font("Arial", Font.BOLD, 15));

            // Load and resize the image
            ImageIcon imageIcon = new ImageIcon(CaseManager.imagePath);
            Image image = imageIcon.getImage();
            Image resizedImage = image.getScaledInstance(200, 200, Image.SCALE_SMOOTH);
            imageIcon = new ImageIcon(resizedImage);

            JLabel imageLabel = new JLabel(imageIcon);
            CaseManagerPanel.setBackground(Color.YELLOW);

            CaseManagerPanel.add(imageLabel, BorderLayout.CENTER);
            CaseManagerPanel.add(nameLabel, BorderLayout.NORTH);
            CaseManagerPanel.add(priceLabel, BorderLayout.SOUTH);

            panel.add(CaseManagerPanel);
        }
        panel.revalidate(); // Refreshing the panel
        panel.repaint(); // Repainting the panel
    }

    // Method to search products by name
    private void searchProducts(String query) {
        List<AirpodCaseManager> filteredCases = airpodCaseManagers.stream()
                .filter(CaseManager -> CaseManager.name.contains(query))
                .collect(Collectors.toList());
        displayProducts(filteredCases);
    }

    // Method to sort products by price
    private void sortProductsByPrice() {
        List<AirpodCaseManager> sortedCases = airpodCaseManagers.stream()
                .sorted(Comparator.comparingDouble(CaseManager -> CaseManager.price))
                .collect(Collectors.toList());
        displayProducts(sortedCases);
    }
}
